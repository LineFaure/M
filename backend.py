...
# -*- coding: utf-8 -*-
"""
Usage: ./bakcend.py& -> run http server in background
This server is in charge of:
- insure that no Y (fork) is possible: the last recipient is the good one
- insure that all credit transactions are used once when the local Bloom Filter returns false positive
...
class handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(s):
        ...
        if   len(d) == 8:  r = s.r8(d) 
        elif len(d) == 20: r = s.r20(d)
        elif len(d) == 292: 
            o.pt = o.uncompress(d[:48])
            if o.verify(d[-96:], d[:-96]): s.w292(d[:8], d[150:154])
        elif len(d) == 300: 
            o.pt = o.uncompress(d[8:56])
            if o.verify(d[-96:], d[8:-96]): s.w300(d[:8] + d[8:16] + d[158:162])
        s.send_response(200)
